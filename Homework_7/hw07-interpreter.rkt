...
(define (value-of ast env)
  (cond 
    [(program? ast) (cases program ast
                      (a-program (vars expr rest-of-expressions)
                       ...
                       (andmap
                        (lambda (ex) (value-of ex (foldl value-of (empty-env) vars)))
...
(define (value-of-expr ex env)
  ...
  (cases expr ex
    ...
    ;we build the anonymous procedure, a procedure boxed by proc-val
    ;with args, body and its current environment packaged inside
    (fun-expr (list-of-identifiers expr) (proc-val (procedure list-of-identifiers expr env)))

    ;use the procedure
    ;unbox it, unbox all its components then
    ;convert the inputs from num-vals into primitives
    ;make the new env with new bindings
    ;evaluate
    ;e := func
    ;args := list of args
    (fun-call-expr (p args)
     (letrec
         ([unboxp (proc-val->proc (value-of p env))]
          [param (proc->vars unboxp)]
          [body (proc->body unboxp)]
          [old-env (proc->env unboxp)]
          [numbers (map (lambda (x) (value-of x env)) args)]
          [new-env (get-new-env param numbers old-env)])
       (value-of body new-env)))    
    (else (raise (~a "value-of-expr error: unimplemented expression: " ex)))
    )
  )

;use foldl to recursively add new params:
;if the amount of inputs is equal to the amount of arguments expected:
;for all inputs:
;bind symbol to primitive, making new env
;fold the default with new and return it.
(define (get-new-env arg numbers env)
  (if (= (length arg) (length numbers))
      (foldl (λ (var val old-env) (extend-env-wrapper var val old-env #t)) env arg numbers)
      (raise (~a "arity mismatch. expected " (length arg) " arguments, received " (length numbers)))))
...
(define (one-at-a-time-add var old-env)
  (cases var-expr var
    (val (iden val-of-iden) (extend-env-wrapper iden (value-of val-of-iden old-env) old-env NON-FINAL))
    (final-val (iden val-of-iden) (extend-env-wrapper iden (value-of val-of-iden old-env) old-env FINAL))
    (fun-def (name args ex) 'a)
))
...
(define (value-of-var v-ex env)
  ...
  (cases var-expr v-ex
    ...
    ;make a new environment, which requires a new procedure that is bound to name
    ;create the new procedure by passing in a proc-val with procedure
    ;procedure uses the arguments from the fun-def (i used idens as the name)
    ;uses the expression on the right hand of the equals sign (ex)
    ;and the old env passed into the value-of
    (fun-def (name idens ex) (extend-env-wrapper name (proc-val (procedure idens ex env)) env #t))
