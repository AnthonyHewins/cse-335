...
(define-datatype expressed-val expressed-val?
  ...
  (step-val (s step?))
  (point-val (p point?))
  ;proc-val is an expressed-val
  (proc-val (l proc?))
  )

;proc takes a list of symbols, and expr, and env
(define-datatype proc proc?
  (procedure (a (list-of symbol?)) (x expr?) (env environment?)))

(define (invalid-args-exception fun-name expected-val actual-val)
  (raise (~a fun-name ", expected: " expected-val ", got: " actual-val) ))
;================================ expressed-val ================================

;extractors, i don't know if all are needed but they're here just in case
...
(define (proc->vars p)
  (cases proc p
      ...
      (else (raise (invalid-args-exception "proc->vars" "proc?" p)))
      )
  )
(define (proc->body p)
  (cases proc p
      (procedure (vars body env) body)
      (else (raise (invalid-args-exception "proc->body" "proc?" p)))
      )
  )
